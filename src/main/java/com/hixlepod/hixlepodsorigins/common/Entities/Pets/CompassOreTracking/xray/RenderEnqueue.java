...
import com.hixlepod.hixlepodsorigins.common.Entities.Pets.CompassOreTracking.*;
...
public class RenderEnqueue implements Runnable {
    ...
    public static void checkBlock(BlockPos pos, BlockState state, boolean add) {
        ...
        if (Controller.getBlockStore().getStore().containsKey(defaultState)) // it's a block we are monitoring
        {
            ...
            double alpha = Math.max(0, ((Controller.getRadius() - HixlePodsOrigins.proxy.getClientPlayer().distanceToSqr(pos.getX(), pos.getY(), pos.getZ())) / Controller.getRadius()) * 255);
                    ...
                    for (int i = lowBoundX; i <= highBoundX; i++) {
                        for (int j = lowBoundY; j <= highBoundY; j++) {
                            for (int k = lowBoundZ; k <= highBoundZ; k++) {
                                ...
                                double alpha = Math.max(0, Controller.getRadius() - HixlePodsOrigins.proxy.getClientPlayer().distanceToSqr(x + i, y + j, z + k) / (Controller.getRadius() / 2));
