...
public class EntitySmudgeNPC extends PathfinderMob implements NeutralMob {


    public EntitySmudgeNPC(EntityType<? extends PathfinderMob> entityType, Level level) {
        super(entityType, level);

        ScaleTypes.WIDTH.getScaleData(this).setScale(0.77f);
        ScaleTypes.HEIGHT.getScaleData(this).setScale(0.75f);
    }
    ...
    public void tick() {
        ...
        if (!this.hasCustomName() || !this.isCustomNameVisible()) {
            this.setCustomName(Component.literal(ChatFormatting.LIGHT_PURPLE + "[Origins] Smudge"));
