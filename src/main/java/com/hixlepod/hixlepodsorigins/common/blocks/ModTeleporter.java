...
public class ModTeleporter implements ITeleporter {
    ...
    public Optional<BlockUtil.FoundRectangle> getExistingPortal(BlockPos pos) {
        ...
        Optional<PoiRecord> optional = poiManager.getInSquare((poiType) ->
                poiType.get() == POIInit.CYBERTRON_PORTAL.get(), pos, 64, PoiManager.Occupancy.ANY).sorted(Comparator.<PoiRecord>comparingDouble((poi) ->
        ...
        return optional.map((poi) -> {
            ...
            this.level.getChunkSource().addRegionTicket(TicketType.PORTAL, new ChunkPos(blockpos), 4, blockpos);
            BlockState blockstate = this.level.getBlockState(blockpos);
            return BlockUtil.getLargestRectangleAround(blockpos, blockstate.getValue(BlockStateProperties.HORIZONTAL_AXIS), 25, Direction.Axis.Y, 25, (posIn) ->
            ...
            for(int l1 = -1; l1 < 2; ++l1) {
                for(int k2 = 0; k2 < 2; ++k2) {
                    for(int i3 = -1; i3 < 3; ++i3) {
                        BlockState blockstate1 = i3 < 0 ? Blocks.IRON_BLOCK.defaultBlockState() : Blocks.AIR.defaultBlockState();
        ...
        for(int k1 = -1; k1 < 3; ++k1) {
            for(int i2 = -1; i2 < 4; ++i2) {
                if (k1 == -1 || k1 == 2 || i2 == -1 || i2 == 3) {
                    ...
                    this.level.setBlock(mutablePos, Blocks.IRON_BLOCK.defaultBlockState(), 3);
                }
            }
        }

        BlockState kaupenPortal = BlockInit.CYBERTRON_PORTAL.get().defaultBlockState().setValue(CybertronPortalBlock.AXIS, axis);
    ...
    protected Optional<BlockUtil.FoundRectangle> getOrMakePortal(Entity entity, BlockPos pos) {
        ...
        if(existingPortal.isPresent()) {
            ...
            Direction.Axis portalAxis = this.level.getBlockState(entity.portalEntrancePos).getOptionalValue(CybertronPortalBlock.AXIS).orElse(Direction.Axis.X);
